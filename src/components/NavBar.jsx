...
export default function Navigation() {
  ...
  const location = useLocation(); // To check active tab
  ...
  // Simplified Nav Items based on new Pages
  const navItems = [
    { path: "/", label: "Home" },
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled || isMobileMenuOpen
            ? "bg-white shadow-md"
            : "bg-white/90 backdrop-blur-md" // Changed transparency logic for readability on all pages
        }`}
      >
          ...
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            <Link to="/" onClick={() => window.scrollTo(0,0)}>
              <div className="w-35 h-70 sm:w-50 sm:h-12 flex items-center justify-center">
                 {/* Use the colored logo always for safety on white backgrounds, or conditional logic */}
                <img
