...
import { Calendar, CheckCircle2, Star, AlertCircle } from 'lucide-react';

const dates = [
  { day: '06', month: 'Feb 2026', label: 'Call for Paper', status: 'upcoming' },
  { day: '10', month: 'Apr 2026', label: 'Full Paper Submission Deadline', deadline: true, status: 'upcoming' },
  { day: '11', month: 'May 2026', label: 'Author Notification', status: 'upcoming' },
  { day: '10', month: 'May 2026', label: 'Camera Ready Submission Deadline Extended', sub: 'Until May 16', status: 'upcoming' },
  { day: '02', month: 'May 2026', label: 'Early Bird Registration Deadline Extended', sub: 'Until May 18', status: 'upcoming' },
  { day: '19', month: 'May 2026', label: 'Regular Registration Opens', sub: 'Until May 23', status: 'upcoming' },
              ...
              {dates.map((item, index) => (
                <div key={index} className="relative group flex flex-col items-center flex-1">
                  ...
                  <div className={`
                    ...
                    ${item.deadline 
                      ? 'bg-gradient-to-br from-rose-500 to-pink-600 border-rose-400 shadow-[0_0_20px_rgba(244,63,94,0.4)]' // Unique Rose style for Deadline
                      : item.highlight
                  ...
                  `}>
                    {/* Icon for Deadline */}
                    {item.deadline && (
                      <div className="absolute -top-3 -right-3 bg-white text-rose-500 p-1.5 rounded-full shadow-md animate-pulse">
                        <AlertCircle className="w-4 h-4 fill-current" />
                      </div>
                    )}

                    {/* Icon for Highlight (Conference) */}
                    {item.highlight && (
                      <div className="absolute -top-3 -right-3 bg-white text-orange-500 p-1.5 rounded-full shadow-md">
                        <Star className="w-4 h-4 fill-current" />
                      </div>
                    )}

                    <span className="block text-4xl font-extrabold mb-1 text-white">
                      {item.day}
                    </span>
                    <span className={`block text-sm font-bold uppercase tracking-wider 
                      ${item.deadline ? 'text-rose-100' : item.highlight ? 'text-amber-100' : 'text-blue-300'}`}>
                      {item.month}
                    </span>
                  </div>
                  ...
                  <div className="relative z-10 flex items-center justify-center w-8 h-8 -mt-12 md:-mt-0 mb-6 transition-all duration-300">
                    <div className={`
                      ...
                      ${item.deadline
                        ? 'bg-rose-400 shadow-[0_0_15px_rgba(244,63,94,0.6)] ring-4 ring-rose-500/20'
                        : item.highlight 
                  ...
                  <div className="text-center transition-colors duration-300">
                    <h3 className={`font-bold text-lg leading-tight mb-1 
                      ${item.deadline ? 'text-rose-400' : item.highlight ? 'text-amber-400' : 'text-white/80 group-hover:text-white'}`}>
                        {item.label}
                    </h3>
                    {item.sub && (
